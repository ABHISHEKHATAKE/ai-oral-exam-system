...
function ViewResults({ token }) {
  ...
  const fetchAllResults = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://ai-oral-exam-system-9.onrender.com/api/instructor/results', {
  ...
  const viewDetailedResult = async (examId) => {
    try {
      const response = await fetch(`https://ai-oral-exam-system-9.onrender.com/api/instructor/results/${examId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
