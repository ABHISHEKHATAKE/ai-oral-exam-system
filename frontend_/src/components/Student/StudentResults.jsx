...
function StudentResults({ token }) {
  ...
  const fetchResults = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await fetch('https://ai-oral-exam-system-9.onrender.com/api/student/results', {
  ...
  const viewDetailedResult = async (examId) => {
    try {
      const response = await fetch(`https://ai-oral-exam-system-9.onrender.com/api/student/results/${examId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
